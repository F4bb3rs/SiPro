def decode(text):
    ...
    for i in text:  #Iteriere alle Zeichen in text
        if i.isalpha(): #Pr?fe, ob Buchstabe ist
            list.append(ord(i.lower()) - 97)    #F?ge der Liste die zugehoerige Zahl zu.
...
def checkKey(a, b):
    if b > 27 or b < 0: #Pr?fe ob der Schluessel b in einem g?ltigen Bereich ist
...
def acEncrypt(a, b, plain_text):
    ...
    return secret.upper()   #Gebe diese Buchstaben als Gro?buchstaben zurueck
